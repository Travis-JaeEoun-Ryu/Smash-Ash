...
public class LivingEntity0 : MonoBehaviourPun, IDamageable {
    public float startingHealth; // 시작 체력
    public float health { get; protected set; } // 현재 체력
    //public float Exp { get; protected set; } // 현재 Exp
    public float monsterExp; // 몬스터 exp

    public float Exp;
    
    public bool dead { get; protected set; } // 사망 상태
    public bool dead1 { get; protected set; } // 사망 상태
    ...
    public virtual void OnDamage(float damage, Vector3 hitPoint, Vector3 hitNormal) {
        ...
        if (health <= 0 && !dead)//health <= 0 && !dead
        {  
            Die();
    ...
    public virtual void GetEXP(float exp) {
            ...
            if (PhotonNetwork.IsMasterClient)
            {
                ...
                Exp += exp;
                photonView.RPC("ApplyUpdatedHealth", RpcTarget.Others, health, dead);
                photonView.RPC("GetEXP", RpcTarget.Others, exp);
